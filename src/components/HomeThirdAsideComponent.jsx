import { Card } from "react-bootstrap";

const HomeThirdAsideComponent = () => {
  return (
    <Card className="mb-2">
      <Card.Body>
        <Card.Title className="text-muted">Accedi a strumenti e informazioni in esclusiva</Card.Title>
        <Card.Text className="text-muted d-flex align-items-center">
          <img
            src=" data:image/png;base64,     "
            alt=""
            width="24"
            height="24"
            className="me-2"
          />
          Prova Premium per 0€
        </Card.Text>
      </Card.Body>
    </Card>
  );
};
export default HomeThirdAsideComponent;
